...
def generate_enzyme_substrate_system(common_product,quantities,labels,rates,vol):
  
  S = Reactant(quantities['S'],labels['S'])
  E = Reactant(quantities['E'],labels['E'])
  SE = Reactant(quantities['SE'],labels['SE'])
  P = common_product
  
  new_reactants = [S,E,SE]
  ...
  return (new_reactants,rx)

def generate_enzyme_production_system(substrate,enzyme,quantities,labels,rates,vol,suppressor=None):

  NG = Reactant(quantities['deactivated genes'],labels['deactivated genes'])
  G = Reactant(quantities['activated genes'],labels['deactivated genes'])

  new_reactants = [NG,G]

  rx = []
  # activation
  # we'll say that availability of substrate causes activation
  rx.append(Reaction({
    'reactants' : [NG,G,substrate],
    'sensitivity list' : [0,2],
    'stoichiometry matrix' : [-1,1,0],
    'rate' : rates['activation'],
    'volume' : vol
  }))
  
  # deactivation - one of the metabolites' deactivation should be controlled by availability of substrate
  # in the future, this could be suppressed by available enzyme
  if suppressor is None:  
    rx.append(Reaction({
      'reactants' : [NG,G],
      'sensitivity list' : [1],
      'stoichiometry matrix' : [1,-1],
      'rate' : rates['deactivation'],
      'volume' : vol
    }))
  else:
    rx.append(Reaction({
      'reactants' : [NG,G,suppressor],
  ...
  # activated gene produces enzyme - maybe it should consume product (cost)?
  rx.append(Reaction({
    'reactants' : [G,enzyme],
    'sensitivity list' : [0],
    'stoichiometry matrix' : [0,1],
    'rate' : rates['enzyme production'],
    'volume' : vol
  }))
